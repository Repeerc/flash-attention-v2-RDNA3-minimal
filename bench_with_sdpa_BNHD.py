...
def sdp_pt(q, k, v=None):
    with torch.backends.cuda.sdp_kernel(
        ...
        q, k, v = map(lambda t: rearrange(t, 'b n h d -> b h n d').contiguous(), (q, k, v))
        r0 = torch.nn.functional.scaled_dot_product_attention(q, k, v, is_causal=causal)
        r0 = rearrange(r0, 'b h n d -> b n h d')
    return r0
...
def sdp_bwd(q, k, v, O, dO):
    dO = rearrange(dO, 'b n h d -> b h n d')
    O = rearrange(O, 'b n h d -> b h n d')
    # q, k, v = map(lambda t: rearrange(t, 'b n h d -> b h n d').contiguous().requires_grad_(True), (q, k, v))
    ...
    # dQ, dK, dV = map(lambda t: rearrange(t, 'b h n d -> b n h d').contiguous(), (dQ, dK, dV))
    return dQ, dK, dV
...
def ftt_rocm(q, k, v):
    ...
    O = wmma_fttn(q,k,v, None, causal, None, True)
...
for i in range(1,11,1):
    ...
    dO2 = dO.clone().detach().requires_grad_(True) 
